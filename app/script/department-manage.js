...
var name;
var action;
var url = "be-department-manage.php";
...
function add(){
	Swal.fire({
		...
		inputValue: '',
		inputPlaceholder: 'ชื่อหน่วยงาน/แผนก',
		reverseButtons: true,
        ...
        confirmButtonText: '<span class="text-light">เพิ่มรายการ</span>',
  		inputValidator: (value) => {
			if (!value) {
				return 'กรุณาระบุชื่อหน่วยงาน/แผนก'
			}else{
				id = "-";
				name = value;
				action = "add";
...
function del(paramID){
	Swal.fire({
		icon: 'warning',
		title: 'ลบรายการ',
		text: "บัญชีผู้ใช้ที่เกี่ยวข้องจะถูกตั้งค่านี้เป็นค่าว่าง",
		reverseButtons: true,
		showCancelButton: true,
		cancelButtonColor: 'white',
